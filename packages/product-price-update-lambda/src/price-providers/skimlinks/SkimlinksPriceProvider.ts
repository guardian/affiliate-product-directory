...
export class SkimlinksPriceProvider extends PriceProvider {
	...
	public async fetchPrices(products: Product[]): Promise<Product[]> {
			...
			for (const batch of this.chunk(byRegion[region], this.batchSize)) {
				...
				const { results } = await this.withRetry(async () => {
					const response = await fetch(endpoint, {
						...
						body: JSON.stringify({
							product_urls: batch.map((product) => product.url),
						}),
					});
					if (!response.ok) {
						throw new Error(
	...
	private updateProducts(
		...
		for (const product of products) {
			const match = productData[product.url]?.[0];
